...
import os, json, time, uuid, duckdb
...
# Load .env for local dev. On Streamlit Cloud, try to pull from st.secrets too.
load_dotenv()
...
def _log_telemetry(inv, step, role, tool, inp, out, tin, tout, lat):
    """Short-lived write connection. No lock held across tool calls."""
    cost = (tin * 3 / 1_000_000 + tout * 15 / 1_000_000) * 100
    con = duckdb.connect(str(DB_PATH))
    try:
        con.execute("""
            INSERT INTO telemetry VALUES (?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)
        """, [inv, step, role, tool,
...
def investigate(anomaly: dict, stream_cb=None):
    ...
    while True:
        ...
        _log_telemetry(inv, step, "assistant", None,
                       {"preview": text_out[:200]},
                       {"n_tools": len(tool_uses)}, tin, tout, lat)
        ...
        for tu in tool_uses:
            ...
            if stream_cb:
                ...
                stream_cb(f"   ↳ {preview}\n")
            _log_telemetry(inv, step, "tool", tu.name,
                           dict(tu.input), result, 0, 0,
    ...
    _save_brief(inv, anomaly["metric"], anomaly["date"],
                final_text, calls, total_cost)
    return {"investigation_id": inv, "brief": final_text,
            "tool_calls": calls, "cost_cents": total_cost}
